...
def test_dataloader(view, batch_size=1, num_workers=0):
...
class TIGREDataset(Dataset):
    ...
    TIGRE dataset.
    """
    def __init__(self, path, n_rays=1024, type="train", retrain=None, device="cuda"):    
        super().__init__()
        ...
        if type == "train":
            ...
            new_angles = [angle + step for angle in angles]

            if retrain is not None:
                tmp_projs = self.projs
                self.projs = torch.cat((self.projs, self.projs))
                tmp_angles = angles
                angles = np.append(angles, angles)
                for i in range(2 * self.n_samples):
                    if i % 2 == 0:
                        self.projs[i] = tmp_projs[i // 2]
                        angles[i] = tmp_angles[i // 2]
                    else:
                        self.projs[i] = retrain[(i - 1) // 2]
                        angles[i] = new_angles[(i - 1) // 2]
                self.n_samples = self.n_samples * 2
            else:
                rays2 = self.get_rays(new_angles, self.geo, device)
                self.rays2 = torch.cat([rays2, torch.ones_like(rays2[...,:1])*self.near, torch.ones_like(rays2[...,:1])*self.far], dim=-1)
